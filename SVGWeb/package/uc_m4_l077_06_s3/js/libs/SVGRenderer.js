...
THREE.SVGRenderer = function () {
	...
	this.render = function ( scene, camera ) {
		...
		scene.traverseVisible( function ( object ) {
			 ...
			 if ( object instanceof THREE.SVGObject ) {
				...
				if(isNaN(x)||x>100000000000000||x<-10000000000000){
 					x = 0
				}
				if(isNaN(y)||y>100000000000000||y<-10000000000000){
 					y = 0
				}
	...
	function renderFace3( v1, v2, v3, element, material ) {
		...
		if ( material.wireframe ) {
			...
			var testmaterialopacity = material.opacity;
			//console.log(testmaterialopacity)
			if(testmaterialopacity!=1&&testmaterialopacity<0.5){
				testmaterialopacity = testmaterialopacity*0.05
			}
			if(testmaterialopacity>=0.5&&testmaterialopacity<0.8){
				testmaterialopacity = testmaterialopacity*0.1
			}
			if(testmaterialopacity>=0.8&&testmaterialopacity<0.9){
				testmaterialopacity = testmaterialopacity*0.2
			}
			//这里是我2015年10月5日修改的  用来遮挡三角形的中线
			//_svgNode.setAttribute( 'style', 'fill: ' + _color.getStyle() + '; fill-opacity: ' + material.opacity+"; stroke-opacity:0"  );
			_svgNode.setAttribute( 'style', 'fill: ' + _color.getStyle() + '; fill-opacity: ' + material.opacity+ '; stroke: ' + _color.getStyle() + '; stroke-width: ' + material.wireframeLinewidth + '; stroke-opacity: ' + (testmaterialopacity) + '; stroke-linecap: ' + material.wireframeLinecap + '; stroke-linejoin: ' + material.wireframeLinejoin );
