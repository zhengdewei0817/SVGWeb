	...
	$.fn.extend({
		zdw_init: function(_KEY_FRAMES, TOTAL_FRAMES) {
			...
			curDom.playAnimation = function(cur){
				$(curDom).css({
					"-moz-transform":curDom.dataframe["fr"+cur],
					"-webkit-transform":curDom.dataframe["fr"+cur],
					"-o-transform":curDom.dataframe["fr"+cur],
					"-ms-transform":curDom.dataframe["fr"+cur],
					"transform":curDom.dataframe["fr"+cur]
				})
				console.log(curDom.dataframe["fr"+cur])
			}
